...
# Convert group column (comma-separated) into lists and check if 'Flying insects' are present (joint category from Diptera and Hymenoptera)
data['is_flying'] = embeds["group"].str.contains('Diptera|Hymenoptera|Insecta', regex=True).astype(int)
# Save to CSV
data.to_csv('is_flying.csv', index=False)
...
df = pd.read_csv("/Users/laradiazgarcia/Desktop/Grenoble/binary_RFC/is_flying.csv")
...
for prefix, files in file_groups.items():
    ...
    label = df[df['filename'] == first_file]['is_flying'].values
    if len(label) > 0:
...
# Save splits
df_train.to_csv('train_annotations_flying.csv', index=False)
df_valid.to_csv('valid_annotations_flying.csv', index=False)

# Print dataset stats
# Count labels in each dataset (using full filename, not prefix-based assignment)
num_train_1 = sum(df_train['is_flying'] == 1)
num_train_0 = sum(df_train['is_flying'] == 0)
num_valid_1 = sum(df_valid['is_flying'] == 1)
num_valid_0 = sum(df_valid['is_flying'] == 0)
...
train_df_with_features = pd.read_csv('train_annotations_flying.csv')
valid_df_with_features = pd.read_csv('valid_annotations_flying.csv') 
X_train = train_df_with_features.drop(columns=["filename", "is_flying"])
y_train = train_df_with_features["is_flying"]
X_valid = valid_df_with_features.drop(columns=["filename", "is_flying"])
y_valid = valid_df_with_features["is_flying"]
...
with open('RFC_accuracy_flying.txt', 'w') as f:
    f.write(classification_report(y_valid, y_pred))
print("Classification report saved to RFC_accuracy_flying.txt")
