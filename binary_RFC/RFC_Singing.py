...
for prefix, files in file_groups.items():
    ...
    label = df[df['filename'] == first_file]['is_singing'].values
    if len(label) > 0:
...
# Save splits
df_train.to_csv('train_annotations_singing.csv', index=False)
df_valid.to_csv('valid_annotations_singing.csv', index=False)
...
with open('RFC_accuracy_singing.txt', 'w') as f:
