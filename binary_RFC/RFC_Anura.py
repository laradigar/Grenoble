...
# Convert group column (comma-separated) into lists and check if 'Anura' is present
data['is_anura'] = embeds["group"].str.contains('Anura').astype(int)
# Save to CSV
data.to_csv('is_anura.csv', index=False)
...
df = pd.read_csv("/Users/laradiazgarcia/Desktop/Grenoble/binary_RFC/is_anura.csv")
...
for prefix, files in file_groups.items():
    ...
    label = df[df['filename'] == first_file]['is_anura'].values
    if len(label) > 0:
...
# Save splits
df_train.to_csv('train_annotations_anura.csv', index=False)
df_valid.to_csv('valid_annotations_anura.csv', index=False)

# Print dataset stats
# Count labels in each dataset (using full filename, not prefix-based assignment)
num_train_1 = sum(df_train['is_anura'] == 1)
num_train_0 = sum(df_train['is_anura'] == 0)
num_valid_1 = sum(df_valid['is_anura'] == 1)
num_valid_0 = sum(df_valid['is_anura'] == 0)
...
train_df_with_features = pd.read_csv('train_annotations_anura.csv')
valid_df_with_features = pd.read_csv('valid_annotations_anura.csv') 
X_train = train_df_with_features.drop(columns=["filename", "is_anura"])
y_train = train_df_with_features["is_anura"]
X_valid = valid_df_with_features.drop(columns=["filename", "is_anura"])
y_valid = valid_df_with_features["is_anura"]
...
with open('RFC_accuracy_Anura.txt', 'w') as f:
    f.write(f"Validation Accuracy: {accuracy:.4f}\n")
    f.write(classification_report(y_valid, y_pred))
print("Classification report saved to RFC_accuracy_Anura.txt")