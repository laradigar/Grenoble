...
# Convert group column (comma-separated) into lists and check if 'Geophony' is present
data['is_geophony'] = embeds["group"].str.contains('Geophony').astype(int)
# Save to CSV
data.to_csv('is_geophony.csv', index=False)
...
df = pd.read_csv("/Users/laradiazgarcia/Desktop/Grenoble/binary_RFC/is_geophony.csv")
...
for prefix, files in file_groups.items():
    label = df[df["filename"] == files[0]]["is_geophony"].values[0]  # Get label
    prefix_labels[prefix] = label
...
valid_prefixes_0 = random.sample(list(valid_prefixes_0), len(valid_prefixes_1))
# valid_prefixes_0 = set(valid_prefixes_0)
...
# Save splits
df_train.to_csv('train_annotations_geophony.csv', index=False)
df_valid.to_csv('valid_annotations_geophony.csv', index=False)

# Print dataset stats
# Count labels in each dataset (using full filename, not prefix-based assignment)
num_train_1 = sum(df_train['is_geophony'] == 1)
num_train_0 = sum(df_train['is_geophony'] == 0)
num_valid_1 = sum(df_valid['is_geophony'] == 1)
num_valid_0 = sum(df_valid['is_geophony'] == 0)
...
train_df_with_features = pd.read_csv('train_annotations_geophony.csv')
valid_df_with_features = pd.read_csv('valid_annotations_geophony.csv') 
X_train = train_df_with_features.drop(columns=["filename", "is_geophony"])
y_train = train_df_with_features["is_geophony"]
X_valid = valid_df_with_features.drop(columns=["filename", "is_geophony"])
y_valid = valid_df_with_features["is_geophony"]
...
predicts.to_csv('predictions_geophony.csv', index=False)

# Load predictions and ground truth
df = pd.read_csv("predictions_geophony.csv")  # Ensure it contains: filename, y_true, y_pred
...
with open('RFC_accuracy_wholeaudio_geophony.txt', 'w') as f:
    f.write("(Prefix-Level Evaluation) \n")
    f.write(f"Validation Accuracy: {accuracy:.4f}\n")
    f.write(classification_report(final_eval_df["y_true"], final_eval_df["y_pred"]))
print("Classification report saved to RFC_accuracy_wholeaudio_geophony.txt")
