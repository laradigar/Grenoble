...
# Save to CSV
df_train.to_csv("train_annotations_aves_balanced.csv", index=False)
df_valid.to_csv("valid_annotations_aves_balanced.csv", index=False)
...
train_df_with_features = pd.read_csv('train_annotations_aves_balanced.csv')
valid_df_with_features = pd.read_csv('valid_annotations_aves_balanced.csv') 
...
predicts.to_csv('predictions_aves_balanced.csv', index=False)

# Load predictions and ground truth
df = pd.read_csv("predictions_aves_balanced.csv")  # Ensure it contains: filename, y_true, y_pred
...
with open('RFC_accuracy_wholeaudio_aves_balanced.txt', 'w') as f:
    f.write("(Prefix-Level Evaluation) \n")
    f.write(f"Validation Accuracy: {accuracy:.4f}\n")
    f.write(classification_report(final_eval_df["y_true"], final_eval_df["y_pred"]))
print("Classification report saved to RFC_accuracy_wholeaudio_aves_balanced.txt")
