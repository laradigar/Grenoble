...
# Convert group column (comma-separated) into lists and check if 'Hymenoptera' is present
data['is_hymenoptera'] = embeds["group"].str.contains('Hymenoptera').astype(int)
# Save to CSV
data.to_csv('is_hymenoptera.csv', index=False)
...
df = pd.read_csv("/Users/laradiazgarcia/Desktop/minimalcode/Archive/RFC/is_hymenoptera.csv")
...
for prefix, files in file_groups.items():
    ...
    label = df[df['filename'] == first_file]['is_hymenoptera'].values
    if len(label) > 0:
...
# Save splits
df_train.to_csv('train_annotations_hymenoptera.csv', index=False)
df_valid.to_csv('valid_annotations_hymenoptera.csv', index=False)

# Print dataset stats
# Count labels in each dataset (using full filename, not prefix-based assignment)
num_train_1 = sum(df_train['is_hymenoptera'] == 1)
num_train_0 = sum(df_train['is_hymenoptera'] == 0)
num_valid_1 = sum(df_valid['is_hymenoptera'] == 1)
num_valid_0 = sum(df_valid['is_hymenoptera'] == 0)
...
train_df_with_features = pd.read_csv('train_annotations_hymenoptera.csv')
valid_df_with_features = pd.read_csv('valid_annotations_hymenoptera.csv') 
X_train = train_df_with_features.drop(columns=["filename", "is_hymenoptera"])
y_train = train_df_with_features["is_hymenoptera"]
X_valid = valid_df_with_features.drop(columns=["filename", "is_hymenoptera"])
y_valid = valid_df_with_features["is_hymenoptera"]
